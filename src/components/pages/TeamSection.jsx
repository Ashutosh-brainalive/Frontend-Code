        ...
        <Slider {...settings}>
          {teamMembers.map((member, idx) => (
            <div key={idx} className="px-2">
              <div className="h-[400px] rounded-[20px] overflow-hidden relative group mx-auto bg-gradient-to-b from-green-700 to-green-900 transition-all duration-300 hover:scale-105">
                <img
                  src={member.image}
                  alt={member.name}
                ...
                <div className="absolute bottom-4 left-1/2 w-[197px] h-[52px] rounded-[24px] bg-black flex items-center justify-between px-2 -translate-x-1/2 z-10 opacity-100 group-hover:opacity-0 transition-opacity duration-300">
                  <div className="bg-green-500 text-white font-semibold text-sm px-3 py-[10px] rounded-[24px] leading-tight text-left w-max whitespace-nowrap overflow-hidden">
                ...
                <div className="absolute inset-0 bg-black bg-opacity-70 flex flex-col justify-end items-center text-center px-4 translate-y-full group-hover:translate-y-0 opacity-0 group-hover:opacity-100 transition-all duration-700 ease-in-out pb-6">
                  <div className="flex items-center gap-2 mb-4">
                    <span className="text-sm bg-green-500 text-black font-bold px-4 py-1 rounded-full animate-slide-up">
                      {member.name}
                    </span>
